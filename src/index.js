...
const CoCreateFloatingLabel = {
	...
	init: function (elements) {
		...
		for (let node of elements) {
			...
			if (
				node.parentNode &&
				!node.parentNode.classList.contains(this.className)
			) {
				this.__wrap(node);
				this.update(node);
			}
	...
	update: function (node, value) {
		if (
			node.classList.contains("floating-label") &&
			node.parentNode.classList.contains("floating-label_field")
		) {
